...
if (trc) {
  ...
  if (tct && tdu && typeof audio !== "undefined") {
    function updateTradTimeDisplay() {
      ...
      tdu.textContent = isNaN(audio.duration) ? "--:--" : formatTime(audio.duration);
    }
    ...
    tct.addEventListener("click", () => {
      if (!isNaN(audio.duration)) {
        const newTime = prompt("Jump to time (seconds):", Math.floor(audio.currentTime));
        if (newTime !== null && !isNaN(newTime)) {
          audio.currentTime = Math.min(Math.max(Number(newTime), 0), audio.duration);
        }
      }
    });
  ...
  function initAudioContext() {
    if (!audioContext) {
      ...
      pitchNode = audioContext.createBufferSource ? null : audioContext.createDelay();
  ...
  ajsb.addEventListener("click", () => {
    if (
      window.confirm(
        "are you sure you want to apply? This will close the window and in turn remove your uploaded songs. Don't apply scripts you dont understand.",
...
if (pitchBendSlider) {
  pitchBendSlider.addEventListener("input", () => {
    ...
    pitchBendStatus.textContent = pitchBend !== 0 ? `Bending: ${pitchBend}` : "";
    updatePitch();
  });
...
function updatePitch() {
  ...
  function updateq() {
    ...
    contextMenu.querySelector("#delete-album").onclick = function () {
     ...
     contextMenu.querySelector("#edit-song").onclick = function () {
      if (contextSongIndex !== null) {
        ...
        // Save changes
        document.getElementById("edit-song-form").onsubmit = async function (e) {
          e.preventDefault();
    ...
    contextMenu.querySelector("#edit-album").onclick = function () {
      if (contextAlbumName) {
        ...
        document.getElementById("edit-album-form").onsubmit = async function (e) {
          e.preventDefault();
          ...
          if (
            ...
            (playlist[currentTrackIndex].album || "Unknown Album") === newAlbumName
          ) {
            initializePlayer(playlist[currentTrackIndex]);
          }
        };
        document.getElementById("close-edit-album-modal").onclick = function () {
          modal.style.display = "none";
        };
      }
      contextMenu.style.display = "none";
  ...
  document.getElementById("delete-playlist").addEventListener("click", function () {
    ...
    if (!confirm(`Are you sure you want to delete the playlist "${playlistName}"? This cannot be undone.`)) return;
    ...
    Object.keys(playlists).forEach((name) => {
      ...
      saveBtn.addEventListener("click", async (e) => {
        ...
        tracks.map(async (track, idx) => {
          ...
          let fileSize = null;
          let filePath = null;
          if (track.file && typeof track.file === "object" && typeof track.file.name === "string") {
        ...
        tracks: serializedTracks
          ...
          zip.file("playlist.soundflareplaylist.json", JSON.stringify(playlistExport, null, 2));
  ...
  // Load Playlist Handler
  document.getElementById("load-playlist").addEventListener("change", async function (e) {
    const file = e.target.files[0];
    if (!file) return;
    try {
      ...
      let newName = playlistImport.meta && playlistImport.meta.name
      ...
      dirInput.addEventListener("change", async function handleDir(e2) {
        ...
        const tracks = await Promise.all(
          playlistImport.tracks.map(async (track) => {
            ...
            if (track.fileName && track.fileSize && track.fileType) {
              matchedFile = fileMap[`${track.fileName}_${track.fileSize}_${track.fileType}`];
            }
        ...
        playlistMeta[newName] = playlistImport.meta || { name: newName, image: "./noart.png" };
        updatePlaylistUI && updatePlaylistUI();

        // Show a summary of missing tracks
        const missingCount = tracks.filter(t => t.missing).length;
        if (missingCount > 0) {
          alert(`Playlist loaded, but ${missingCount} track(s) could not be matched to files in the selected directory.`);
        } else {
      ...
      setTimeout(() => {
        try {
          ...
          alert("Please click the directory picker to select your music folder.");
          document.body.appendChild(dirInput);
        }
      }, 0);
    } catch (err) {
  ...
  function initializePlayer(track) {
    ...
    window.electronAPI.send("update-rpc", {
...
window.electronAPI.onTaskbarPrevious(() => {
  playPrevious();
});
window.electronAPI.onTaskbarPlayPause(() => {
  play.click();
});
window.electronAPI.onTaskbarNext(() => {
  playNext();
});
